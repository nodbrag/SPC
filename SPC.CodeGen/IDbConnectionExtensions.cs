...
namespace SPC.CodeGen
{
    ...
    public static class IDbConnectionExtensions
    {
        ...
        /// <returns></returns>
        public static List<DbTable> GetCurrentDatabaseTableList(this IDbConnection dbConnection, Core.DbHelper.DatabaseType dbType)
        {
        ...
        private static List<DbTable> GetCurrentDatabaseAllTables(this IDbConnection dbConnection, Core.DbHelper.DatabaseType dbType)
        {
        ...
        /// <returns></returns>
        private static List<DbTableColumn> GetColumnsByTableName(this IDbConnection dbConnection, Core.DbHelper.DatabaseType dbType, string tableName)
        {
            if (dbConnection == null) throw new ArgumentNullException(nameof(dbConnection));
            if (dbConnection.State == ConnectionState.Closed) dbConnection.Open();
        ...
        private static string strGetAllTablesSql(this IDbConnection dbConnection, Core.DbHelper.DatabaseType dbType)
        {
            string strGetAllTables = string.Empty;
            switch (dbType)
            {
                case Core.DbHelper.DatabaseType.SqlServer:
...
FROM      sys.syscolumns AS a LEFT OUTER JOIN
                ...
                sys.extended_properties AS f ON d.id = f.major_id AND f.minor_id = 0";
                    break;
                case Core.DbHelper.DatabaseType.MySQL:
                    strGetAllTables =
                    ...
                    break;
                case Core.DbHelper.DatabaseType.PostgreSQL:
                    strGetAllTables =
        ...
        private static string strGetAllColumnsSql(this IDbConnection dbConnection, Core.DbHelper.DatabaseType dbType, string tableName)
        {
